...
class PSO():
    ...
    def initializeBirds(self):
        ...
        self.bestFitness = 1.7976931348623157E+308; #from C# Max double Value
        for bird in range(len(self.birdsFlock)):
            ...
            #self.birdsFlock[bird] = Bird(randPositions, randVelocity, randPositions, error, error);
    ...
    def objectiveFunction(self, x):
        ...
        # x[0] * exp(-(x[0] * x[0] + x[1] * x[1])) Success!
        return ((3.0 + (x[0] * x[0]) + (x[1] * x[1]))); #Success
        ...
        for it in range(1, Globals.NUM_OF_ITERATIONS):
            ...
            for bird in range(self.birdsFlock.__len__()):
                ...
                if dieProb < Globals.DEATH_PROBABILITY:
                    self.birdsFlock[bird].position[0] = fabs(Globals.MAXIMUM_X - Globals.MINIMUM_X) * random.random() + Globals.MINIMUM_X;
                    self.birdsFlock[bird].position[1] = fabs(Globals.MAXIMUM_X - Globals.MINIMUM_X) * random.random() + Globals.MINIMUM_X;
                    self.birdsFlock[bird].bestPosition[0] = self.birdsFlock[bird].position[0];
                    self.birdsFlock[bird].bestPosition[1] = self.birdsFlock[bird].position[1];
